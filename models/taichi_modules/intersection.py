...
@ti.kernel
def ray_aabb_intersec_taichi_forward(
        hits_t: ti.types.ndarray(field_dim=3),
hits_voxel_idx: ti.types.ndarray(field_dim=2),
       hit_cnt: ti.types.ndarray(field_dim=1),
    ...
    half_sizes: ti.types.ndarray(field_dim=2),
      max_hits: ti.i32):
    for r, v in hits_voxel_idx:
        ray_o = vec3([rays_o[r, 0], rays_o[r, 1], rays_o[r, 2]])
        ray_d = vec3([rays_d[r, 0], rays_d[r, 1], rays_d[r, 2]])
        inv_d = 1.0 / ray_d

        center = vec3([centers[v, 0], centers[v, 0], centers[v, 0]])
        half_size = vec3([half_sizes[v, 0], half_sizes[v, 0], half_sizes[v, 0]])    
        ...
        if t2 > 0.0:
            cnt = ti.atomic_add(hit_cnt[r], 1)
            if cnt < max_hits:
                hits_t[r, cnt, 0] = ti.max(t1, NEAR_DISTANCE)
                hits_t[r, cnt, 1] = t2
...
def simple_ray_aabb_intersec_taichi_forward(
    ...
    for r in ti.ndrange(hits_t.shape[0]):
        ...
        half_size = vec3([half_sizes[0, 0], half_sizes[0, 1], half_sizes[0, 1]])    
...
class RayAABBIntersector(torch.autograd.Function):
    ...
    def forward(ctx, rays_o, rays_d, center, half_size, max_hits):
        hits_t = (torch.zeros(rays_o.size(0), 1, 2, device=rays_o.device, dtype=torch.float32) - 1).contiguous()
        ...
        simple_ray_aabb_intersec_taichi_forward(hits_t, rays_o, rays_d, center, half_size)
