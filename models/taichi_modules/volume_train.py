...
class VolumeRenderer(torch.nn.Module):
    ...
    def __init__(self, batch_size=8192):
        ...
        # samples level
        self.sigmas_fields = ti.field(dtype=data_type, shape=(batch_size*1024, ), needs_grad=True)
        self.rgbs_fields = ti.field(dtype=data_type, shape=(batch_size*1024, 3), needs_grad=True)
        self.deltas_fields = ti.field(dtype=data_type, shape=(batch_size*1024,), needs_grad=True)
        self.ts_fields = ti.field(dtype=data_type, shape=(batch_size*1024,), needs_grad=True)
        self.ws_fields = ti.field(dtype=data_type, shape=(batch_size*1024, ), needs_grad=True)
        self.T = ti.field(dtype=data_type, shape=(batch_size*1024), needs_grad=True)
        ...
        self.total_samples_fields = ti.field(dtype=ti.i64, shape=(batch_size,))
        self.opacity_fields = ti.field(dtype=data_type, shape=(batch_size,), needs_grad=True)
        self.depth_fields = ti.field(dtype=data_type, shape=(batch_size,), needs_grad=True)
        self.rgb_fields = ti.field(dtype=data_type, shape=(batch_size, 3), needs_grad=True)
        ...
        class _module_function(torch.autograd.Function):
            ...
            def forward(ctx, sigmas, rgbs, deltas, ts, rays_a, T_threshold):
                ...
                total_samples = torch.zeros(
                    rays_size, dtype=torch.int64, device=rays_a.device
                )
                rgb = torch.zeros(
                    rays_size, 3, dtype=torch_type, device=rays_a.device
                )
                opacity = torch.zeros(rays_size, dtype=torch_type, device=rays_a.device)
                depth = torch.zeros(rays_size, dtype=torch_type, device=rays_a.device)
            ...
            def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drgb, dL_dws):
                ...
                sigma_grad = torch.zeros(samples_size, dtype=torch_type, device=dL_dws.device)
                rgb_grad = torch.zeros(samples_size, 3, dtype=torch_type, device=dL_dws.device)
