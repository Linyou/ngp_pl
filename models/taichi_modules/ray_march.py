...
def raymarching_train(
    ...
    density_bitfield: ti.types.ndarray(field_dim=1),
            ...
            cascades: int, grid_size: int, 
               scale: float, exp_step_factor: float,
         max_samples: float):
...
@ti.kernel
def raymarching_train_backword(
      segments: ti.types.ndarray(field_dim=2),
            ts: ti.types.ndarray(field_dim=1),
...
class RayMarcher(torch.autograd.Function):
    ...
    def forward(ctx, rays_o, rays_d, hits_t,
        ...
        counter = torch.zeros(2, device=rays_o.device, dtype=torch.int32)
        rays_a = torch.zeros(rays_o.size(0), 3, device=rays_o.device, dtype=torch.int32)
        xyzs = torch.zeros(rays_o.size(0)*max_samples, 3, device=rays_o.device, dtype=rays_o.dtype)
        dirs = torch.zeros(rays_o.size(0)*max_samples, 3, device=rays_o.device, dtype=rays_o.dtype)
        deltas = torch.zeros(rays_o.size(0)*max_samples, device=rays_o.device, dtype=rays_o.dtype)
        ts = torch.zeros(rays_o.size(0)*max_samples, device=rays_o.device, dtype=rays_o.dtype)

        raymarching_train(
                rays_o, rays_d, hits_t.contiguous(),
                density_bitfield, noise, counter,
                rays_a, xyzs, dirs, deltas, ts,
                cascades, grid_size, scale, exp_step_factor, max_samples)
    ...
    def backward(ctx, dL_drays_a, dL_dxyzs, dL_ddirs,
        ...
        raymarching_train_backword(
            segments.contiguous(), ts, dL_drays_o, dL_drays_d, dL_dxyzs, dL_ddirs)
        # ti.sync()
...
def raymarching_test(    
    ...
    xyzs = torch.zeros(N_rays, N_samples, 3, device=rays_o.device, dtype=rays_o.dtype)
    dirs = torch.zeros(N_rays, N_samples, 3, device=rays_o.device, dtype=rays_o.dtype)
    deltas = torch.zeros(N_rays, N_samples, device=rays_o.device, dtype=rays_o.dtype)
    ts = torch.zeros(N_rays, N_samples, device=rays_o.device, dtype=rays_o.dtype)
    N_eff_samples = torch.zeros(N_rays, device=rays_o.device, dtype=torch.int32)

    raymarching_test_kernel(
        rays_o, rays_d, hits_t, 
        alive_indices, density_bitfield, 
        cascades, grid_size, scale, exp_step_factor, N_samples, max_samples, 
        xyzs, dirs, deltas, ts, 
        N_eff_samples)
